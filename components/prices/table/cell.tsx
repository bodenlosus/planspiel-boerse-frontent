...
export default function PriceCell({ columnIndex, value, className, options }: PriceCellProps) {
    ...
    if (options.indicator && isNumber) {
      return (
        ...
        >
          ...
          <span>{get_sign(value as number)}{(options.type === 'float') ? formatFloatingPointString(Math.abs(value as number), 2): value}</span>
        </TableCell>
      );
    }
    ...
    >
      {(options.type === 'float') ? formatFloatingPointString(value as number, 2): value}
    </TableCell>
  );
}
